...
# -*- coding: utf-8 -*-
"""
Clash 节点筛选器 - v6.0 (实用保底版)
策略：真实测速 + TCP 保底，确保至少有可用节点
"""

import requests, base64, hashlib, time, json, socket, os, sys, re, yaml, subprocess, signal, gzip, shutil
...
# ⭐ 宽松阈值
MAX_LATENCY = 1500
MIN_PROXY_SPEED = 0.01  # 0.01 MB/s = 80 Kbps
MAX_PROXY_LATENCY = 3000
TCP_FALLBACK_TARGET = 50  # 测速不足时用 TCP 补充到 50 个
...
class NodeNamer:
    ...
    def to_fancy(self, t): return ''.join(self.FANCY.get(c,c) for c in t)
    def generate(self, flag, lat, speed=None, tcp=False):
        ...
        if speed: return f"{code}{self.counters[code]}-{pfx}|⚡{lat}ms|📥{speed:.1f}MB"
        return f"{code}{self.counters[code]}-{pfx}|⚡{lat}ms{'(TCP)' if tcp else ''}"
...
class ClashManager:
    ...
    def download_clash(self):
        ...
        try:
            ...
            with gzip.open(temp, "rb") as f_in:
                with open(CLASH_PATH, "wb") as f_out: shutil.copyfileobj(f_in, f_out)
    ...
    def create_config(self, proxies):
        ...
        with open(CONFIG_FILE, "w", encoding="utf-8") as f: yaml.dump(config, f, allow_unicode=True)
        return True
    ...
    def start(self):
        ...
        try:
            ...
            for i in range(30):
                ...
                try:
                    if requests.get(f"http://127.0.0.1:{CLASH_API_PORT}/version", timeout=2).status_code == 200: return True
                except: pass
            return False
        except: return False
    ...
    def stop(self):
        if self.process:
            try: os.killpg(os.getpgid(self.process.pid), signal.SIGTERM); self.process.wait(timeout=5)
            except: pass
    ...
    try:
        ...
        for _ in range(2):
            try:
                ...
                if d.startswith("{"): payload = d; break
                payload = d
            except: break
    ...
    try:
        ...
        if sec in ["tls","reality"]:
            ...
            if sec == "reality":
                ...
                else: return None  # Reality 参数缺失，丢弃
            fp = gp("fp")
            if fp: proxy["client-fingerprint"] = fp
            else: proxy["client-fingerprint"] = "chrome"
        flow = gp("flow")
...
def get_region(name):
    ...
    elif any(k in nl for k in ["th","thailand","泰"]): return "🇹🇭","TH"
    elif any(k in nl for k in ["vn","vietnam","越"]): return "🇻🇳","VN"
    elif any(k in nl for k in ["us","usa","美"]): return "🇺🇸","US"
    elif any(k in nl for k in ["uk","britain","英"]): return "🇬🇧","UK"
    elif any(k in nl for k in ["de","german","德"]): return "🇩🇪","DE"
    elif any(k in nl for k in ["fr","france","法"]): return "🇫🇷","FR"
    elif any(k in nl for k in ["nl","netherlands","荷"]): return "🇳🇱","NL"
    return "🌍","OT"

def is_base64(s):
    try:
        s = s.strip()
        if len(s)<10 or not re.match(r'^[A-Za-z0-9+/=]+$',s): return False
...
def fetch(url):
    try: return requests.get(url,headers=HEADERS,timeout=TIMEOUT).text.strip()
    except: return ""
...
def check_url(u):
    try: return requests.head(u,headers=HEADERS,timeout=TIMEOUT,allow_redirects=True).status_code in (200,301,302)
    except: return False
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=30) as ex:
            ...
            for i,f in enumerate(as_completed(futs)):
                p,lat,asia = f.result()
                if lat < MAX_LATENCY: nres.append({"proxy":p,"latency":lat,"is_asia":asia})
        ...
        if len(nres) > 0:
            ...
            if clash.create_config(tprox) and clash.start():
                ...
                # ⭐ TCP 保底
                if len(final) < TCP_FALLBACK_TARGET:
                    print(f"\n⚠️ 测速合格 {len(final)} 个，使用 TCP 补充到 {TCP_FALLBACK_TARGET} 个...")
                    for item in nres:
                        if len(final) >= TCP_FALLBACK_TARGET: break
                        p = item["proxy"]
                        k = f"{p['server']}:{p['port']}"
        ...
        with open("proxies.yaml","w",encoding="utf-8") as f: yaml.dump(cfg,f,allow_unicode=True,default_flow_style=False)
        
        b64 = base64.b64encode("\n".join([f"# {p['name']}" for p in final]).encode()).decode()
        with open("subscription_base64.txt","w",encoding="utf-8") as f: f.write(b64)
