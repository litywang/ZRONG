#!/usr/bin/env python3
"""v30.0 Phase 1.2: main_flow.py 阶段级超时控制 + 渐进式输出"""

PATH = 'core/main_flow.py'

with open(PATH, 'r', encoding='utf-8') as f:
    content = f.read()

# Replace the entire main() function with stage-level timeouts + incremental output
old_main_start = 'def main():'
idx = content.find(old_main_start)
if idx == -1:
    print("ERROR: Cannot find main()")
    exit(1)

# Find the end of main() (next top-level def or end of file)
rest = content[idx + len(old_main_start):]
next_def = rest.find('\ndef ')
if next_def == -1:
    next_def = len(rest)
old_main = content[idx:idx + len(old_main_start) + next_def]
...
content = content[:idx] + new_main + content[idx + len(old_main):]
print(f"Replaced main() function")

with open(PATH, 'w', encoding='utf-8') as f:
    f.write(content)

print("OK: core/main_flow.py updated")
